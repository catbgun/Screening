...
- har det no å si om det er mellomrm mellom < og tallet? nei, tror ikke det
```{r}
...
#df <- read.table("230914_CLEAN_Results with format_Screening2022.txt", skip=5, header=TRUE, sep="\t", na.string=c(""))
...
df_longgg = df_longg%>%
  mutate(Medium_id2 = case_when(Medium_id %in% c("Water", "Water_filtered", "Water_particle") ~ "Water",
...
```{r Heatmap table chl}
ggplot(subset(chl, Medium_id2%in% c("Water")), aes(y=compound, x=Site_Category)) 
ggplot(surf, aes(y=compound, x=Site_Category)) +
  geom_tile(aes(fill = det_frac), colour="gray", width = 2, height = 2)+  # background colours are mapped according to the value column
  geom_text(aes(label = round(mean, 1)), color = "white", size = 6)+ # write the values
  scale_fill_gradient2(low = "green", 
                       mid = "orange", 
                       high = "red",
                       midpoint = 50,
                       breaks=c(0,50,100), limits=c(0,100))+  # determine the colour
  theme(panel.grid.major.x=element_blank(), #no gridlines
        panel.grid.minor.x=element_blank(), 
        panel.grid.major.y=element_blank(), 
        panel.grid.minor.y=element_blank(),
        panel.background=element_rect(fill="white"), # background=white
        axis.text.x = element_text(angle=45, hjust = 1,vjust=1,size = 18,colour="black"),
  ...
  ggtitle("Water (ng/L) and Sludge (ng/g)") + 
  theme(legend.title=element_text(size=14)) + 
  scale_x_discrete(name="") +
  scale_y_discrete(name="") +
  labs(fill="Detection frequency (%)")+
  facet_grid(Medium_id2~., scales="free_y")
  coord_fixed()
```
