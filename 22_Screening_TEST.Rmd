...
df <- read.table("V02_CLEAN_Results with format_Screening2022_231005_3.txt", skip=6, header=TRUE, sep="\t", na.string=c(""))

#df <- read.table("V02_CLEAN_Results with format_Screening2022_231005_2.txt", skip=6, header=TRUE, sep="\t", #na.string=c(""))

#df <- read.table("V02_CLEAN_Results with format_Screening2022_231003.txt", skip=5, header=TRUE, sep="\t", na.string=c(""))
...
print(unique(df_longg))

df_longgg = df_longg%>%
  mutate(Medium_id2 = case_when(Medium_id %in% c("Water, total", "Water, filtered", "Water, particles") ~ "Water",
...
df_long_X <- cbind(tosty[,c(1:3,9:14,18:23)], apply(tosty[,24, drop=F],2, dLimity))
...
unique(bis$compound)
 ...
 #AIR AND SOLIDS
bis1 <-ggplot(subset(bis, Medium_id2%in% c("Water")), aes(x=Medium_id, y=fct_rev(compound), fill=det_frac)) +
