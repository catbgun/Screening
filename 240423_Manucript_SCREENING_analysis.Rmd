...
df2$Site_Category <- revalue(df2$Site_Category, c("Artificial grass pitch" = "Artificial football pitch"))
...
sum <- df4_finalx %>% 
  group_by(Medium_id, compound, Site_Category) %>% 
  summarise(
    mean = mean(plotting, na.rm = TRUE),
    min  = min(plotting, na.rm = TRUE),
    max  = max(plotting, na.rm = TRUE),
    sd   = sd(plotting, na.rm = TRUE),
    median  = median(plotting, na.rm = TRUE),
    .groups = "drop"
  )
...
sum_n <- df4_finalx %>% 
  group_by(Medium_id, compound, Site_Category) %>%
  summarise(
    n = sum(!is.na(plotting)),
    .groups = "drop"
  )

nrow(df4_finalx)

summary_tb <- merge(sum, sum_n, by = c("Medium_id", "compound", "Site_Category"))

#to also include n and detection frequency
df_unique <- df4_finalx %>% distinct(Medium_id, compound, Site_Category, .keep_all = TRUE)

#merge
summary_tbx <- merge(summary_tb, df_unique, by = c("Medium_id", "compound", "Site_Category"))


write.csv(summary_tbx, "260610_Scr22_MS_summary table.csv", row.names=FALSE)



table<- df4_finalx %>% 
  group_by(Medium_id, compound, Site_Category) %>% 
  summarise(plotting, .f = list(mean = mean, min = min, max = max, sd = sd, median=median), na.rm = TRUE)
...
aa4x_source <- subset(df4_finalx, Site_Category %in% c("Road environment","Artificial football pitch", "Wastewater treatment plant"))
...
aa4x_source3xy <- aa4x_source3y%>% 
  mutate(Site_Category = factor(Site_Category, levels =c("Artificial football pitch", "Road environment","Wastewater treatment plant")))
...
#print(unique(aa4x_source3x$Group))

aa4x_source3x$Medium_id <- factor(
  aa4x_source3x$Medium_id,
  levels = c("Crumb rubber (ng/g, w.w.)", "Water (ng/L)", "Sludge (ng/g, d.w.)", "PM10 (ng/m^3)"),
  labels = c(
    "Crumb rubber\n(ng/g, w.w.)",
    "Water\n(ng/L)",
    "Sludge\n(ng/g, d.w.)",
...
aa4x_source3x$Group <- factor(
  aa4x_source3x$Group,
  levels = c("Benzothiazoles", "Substituted para-phenylenediamines", "Various others"),
  labels = c(
    "Benzothiazoles", 
...
high = "#C44E52",
   ...
   theme(panel.grid.major.x=element_blank(), #no gridlines
        ...
        plot.margin = margin(1.3, 0.2, 0.15, 0.15, "cm"), #top, right, bottom, left plot.margin = margin(1.3, 0.2, 0.15, 0.15, "cm")plot.margin = margin(1.3, 0.2, 2.8, 0.15, "cm"), #top, right, bottom, left
        legend.margin=margin(0,0,0,0),
        legend.box.margin=margin(-40,-210,140,10), #top, right, bottom, left
  ...
  facet_grid(Group~Medium_id, scales="free", space="free_y", labeller = label_value)
  

#facet_grid(Group~., scales="free", space="free_y", labeller = label_wrap_gen(width=2))

# --- PARAMETERS (tune once) ---
cell_h <- 0.7   # height per compound
cell_w <- 1.4    # width per x cell

# --- HEIGHT: only depends on compounds ---
n_compounds <- length(unique(aa4x_source3x$compound))
plot_height <- n_compounds * cell_h

# --- WIDTH: depends on x structure + facets ---
n_x_total <- sum(
  tapply(aa4x_source3x$Site_Category,
         aa4x_source3x$Medium_id,
         function(x) length(unique(x)))
)

plot_width <- n_x_total * cell_w
...
#print(unique(aa4x_rec2x$Medium_id))
aa4x_rec2x$Medium_id <- factor(
  aa4x_rec2x$Medium_id,
  levels = c("Water (ng/L)", "Sediment (ng/g, d.w.)", "PM10 (ng/m^3)"),
  labels = c(
   "Water\n(ng/L)", 
   "Sediment\n(ng/g, d.w.)", 
   "PM10\n(ng/m^3)"
...
high = "#C44E52",
   ...
   theme(panel.grid.major.x=element_blank(), #no gridlines
        ...
        legend.position =c(0.85, 0.96),
        legend.direction = "horizontal",
        plot.margin = margin(1.3, 0.2, 2.8, 0.15, "cm"), #top, right, bottom, left
        legend.margin=margin(0,0,0,0),
        legend.box.margin=margin(-40,-210,140,10),
        strip.text.x = element_text(size = 15),
        panel.spacing.x = unit(0.2, "lines"),
        legend.title=element_text(size=12), 
        strip.text.y = element_text(angle = 0, size=13)) +
  ...
  scale_y_discrete(name="") +
  labs(fill="Detection frequency (%)")+
  facet_grid(Group~Medium_id,  space="free_y", scales="free", labeller = label_value) #scales="free",
...
# --- HEIGHT: only depends on compounds ---
n_compounds <- length(unique(aa4x_rec2x$compound))
plot_height <- n_compounds * cell_h
...
#ggsave("rec_sites.png", rec_sites, width = 6, height = 8)
...
#fjern luftprøver fotballbane grunnet NAs og granulatprøver
...
corry3 <- corry2 %>% 
  rename("Henry's law constant" = "HENRYS_LAW_ATM.M3.MOLE_OPERA_PRED_log", 
        ...
        "Water_Football"= "Water (ng/L)_Artificial football pitch_log",
        ...
        "Sludge_Football" ="Sludge (ng/g, d.w.)_Artificial football pitch_log",
...
ggsave("PCA_biplot.tiff", bi, width = 8.5, height = 7.5)
...
pxp <- ggpaired(sht7, cond1 = "mean_valueU", cond2 = "mean_valueT",
  ...
  geom_boxplot(fill = c("lightblue3", "darksalmon", "lightblue3", "darksalmon"), colour="darkgray") +
...
ggsave("treatment.png", pxp, width = 7, height = 7)

```

PNEC FIGUE
```{r Making the PNEC Figure}
#unique(pnec$compound)  
print(unique(df4_finalx$Medium_id))
#some problem with categorical variables, water turned into na. Here fix:
df4_finalx$Medium_id2 <- ifelse(df4_finalx$Medium_id == "Water (ng/L)", 'Freshwater',
                  ifelse(df4_finalx$Medium_id == "Sediment (ng/g, d.w.)", 'Sediment',
                         ifelse(df4_finalx$Medium_id == "Sludge (ng/g, d.w.)", 'Sediment',
                                ifelse(df4_finalx$Medium_id == "PM10 (ng/m^3)", 'Air', "CBG"))))

#All "< LOD" converted to NA to avoid these from being included
df4_finalx$value2 <- as.numeric(df4_finalx$value)
pnec <- df4_finalx %>% drop_na(value2)

#remove irrelevant sample category and sample types without PNEC
pnec1 <- pnec %>% 
...
pnec3 <- subset(pnec2, !Medium_id2=="CBG") #CBG covers air-particles and crumb rubber

#get min and max measured values by each sample type and compound
pnec_max_minx <- pnec3 %>%
  group_by(compound, Medium_id2) %>%
  mutate(Min = min(value2), Max = max(value2))

#change names to match pnec df
pnec_max_min2 <- pnec_max_minx%>%
  mutate(sample_category = case_when
         (Medium_id2 %in% c("Freshwater") ~ "Freshwater",
...
dx3qqqx$sample_category[dx3qqqx$sample_category == "Freshwater2"] <- "Freshwater"
dx3qqqx$sample_category[dx3qqqx$sample_category == "Sediment2"] <- "Sediment"

dx3qqqx<- dx3qqqx %>%
  filter(compound != "IPPD")

pnec <- ggplot(dx3qqqx, aes(y=fct_rev(compound), x=value2, xmin = Min2, xmax=Max2, colour = sample_category))+ 
  geom_linerange(size=3, aes(colour = sample_category, alpha=1), position = position_dodge2(width = 0.7))+
  ...
  xlab("Range of risk quotients (log scale)")+
  ...
  geom_vline(xintercept = 1, linetype="dotted", 
                color = "red", size=1.8)+
  facet_grid(Group~sample_category, scales="free_y", space="free_y", labeller = label_wrap_gen(width=2))

ggsave("pnec.tiff", pnec, width = 8, height = 6.5)
...
#envixx2 <- envixx2x %>% 
#  mutate(Site_name = factor(Site_name, levels = c("Kalbakken", "Brubak", "Breivoll", "KvÃ¦rner")))
...
To DO: rekkefÃ¸lge medium, gruppering stoffene, etc. 
...
envixx5c <- envixx4c %>% 
  mutate(Site_name = factor(Site_name, levels = c("Kalbakken", "Brubak", "Breivoll", "Kvâ€˜rner")))

envixx5c$Site_name <- revalue(envixx5c$Site_name, c("Kvâ€˜rner"="KvÃ¦rner"))
