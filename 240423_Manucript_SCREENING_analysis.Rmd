...
aa4x_source3 <- aa4x_source%>% 
  mutate(Medium_id = factor(Medium_id, levels = c("Granule (ng/g, w.w.)", "Water (ng/L)", "Sludge (ng/g, d.w.)", "Air, particles (ng/m^3)")))
...
aa4x_source3x = aa4x_source3xy%>%
  mutate(Group = case_when(compound %in% c("DPG", "HMMM", "TMQ") ~ "Various", #skal vi kalle det noe annet?
...
ggplot(footxx, aes(y=value, x=compound, fill=Comment_location))+
  geom_point(size=5, aes(shape=Comment_location, colour=Comment_location))+
  scale_y_continuous(trans='log10')
...
pnec_max_minz <- subset(pnec_max_miny, !Medium_id2=="CBG")

unique(pnec_max_minz$Medium_id2)

pnec_max_min2 <- pnec_max_minz%>%
