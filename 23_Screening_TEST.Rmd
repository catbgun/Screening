...
df23 <- read.table("240702_240528_SCRE2023_Resultater.txt", skip=1, header=TRUE, sep="\t", na.string=c(""))
...
df23_longggz = df23_longgg%>%
  mutate(Part = case_when(Sample_type %in% c("Wastewater treatment", "Unknown hotspot", "Known hotspot") ~ "Part1",
...
newtable <- merge(df23_longggz,gr23, by.x  = "compound", by.y="ID") 
...
df23_long_X <- cbind(tosty23[,c(1:7,9:14 )], apply(tosty23[,8, drop=F],2, dLimity))
