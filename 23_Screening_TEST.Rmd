...
part1_filt_ww1 <- cbind(part1_filt_ww[,c(1:45)], apply(part1_filt_ww[,46, drop=F],2, dLimity))
...
WWT_treatment <- ggplot(part1_final_ww5, aes(y=(meanx), x=fct_rev(compound), fill=Treatmen))+
...
dust3 <- cbind(dust2[,c(1:45)], apply(dust2[,46, drop=F],2, dLimity))
...
prod3 <- cbind(prod2[,c(1:45)], apply(prod2[,46, drop=F],2, dLimity))
...
products <- ggplot(prod5, aes(y=(meanx), x=compound, fill=Treatmen))+
  #geom_col(position="dodge")+
...
ggplot(topP6z, aes(x=compound, y=plotting, colour=Level, shape=Level))+
  geom_point(size=3.5)+
  theme_bw()+
