...
# claculate 0.5xLOD 
part1_filt_X <- cbind(part1_filt[,c(1:44)], apply(part1_filt[,45, drop=F],2, dLimity))
...
part1_finalxy <- part1_finalx %>% 
  mutate(meanxyz = ifelse(compound == "BHPMEBD" |compound == "DMPTDPP" | compound =="DMPTTPP"
                          | compound =="DIUDP"| compound =="B-2ETF",
                           "S", 
                           " "))
...
gju <- subset(hotspot2, compound=="BHPMEBD")
...
#to get "s" for suspect and "n.a." for not available
part1_final_ww2a <- subset(part1_final_ww2, !compound %in% c("BHPMEBD","DMPTDPP","DMPTTPP", "DIUDP","B-2ETF"))
...
ggplot(part1_final_ww5, aes(y=(meanx), x=compound, fill=Treatmen))+
...
#to get "s" for suspect and "n.a." for not available
dust4a <- subset(dust4, !compound %in% c("BHPMEBD","DMPTDPP","DMPTTPP", "DIUDP","B-2ETF"))
...
ggplot(dust6, aes(y=(meanx), x=compound, fill=Treatmen))+
  geom_col(position = position_dodge2(width = 0.9, preserve = "single"))+
...
#to get "s" for suspect and "n.a." for not available
prod4x <- subset(prod4, !compound %in% c("BHPMEBD","DMPTDPP","DMPTTPP", "DIUDP","B-2ETF"))
...
#to get "s" for suspect and "n.a." for not available
part2_final_3 <- subset(part2_final_2, !compound %in% c("BHPMEBD","DMPTDPP","DMPTTPP", "DIUDP","B-2ETF"))
...
ggplot(topP5, aes(x=compound, y=log(plotting), colour=Level))+
  geom_point(size=2.6)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
