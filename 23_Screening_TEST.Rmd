...
```{r Part1 Wastewater treatment}
ggplot(subset(part1_finalxyz, Category%in% c("Wastewater treatment")), aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
...
ggplot(subset(part1_finalxyz, Category%in% c("Known hotspot", "Unknown hotspot")), aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
  geom_tile(aes(fill = det_frac), colour="black")+  # background colours are mapped according to the value column
  geom_text(aes(label=ifelse(is.na(meanx), "", sigfig(meanx, 1))), color = "white", size = 5)+
 scale_fill_gradient2(low = "green", 
                       mid = "orange", 
