...
part1xx <- subset(part1x, Sample.type == c("Water. filtered", "Water. particulate", "Sludge"))
...
part1_filt_ww <- cbind(part1_filt_ww[,c(1:23)], apply(part1_filt_ww[,24, drop=F],2, dLimity))

part1_final_ww1 = part1_filt_ww %>%
  group_by(Category, compound, Sample.type, Treatmen) %>%
  mutate(meanx = mean(plotting, na.rm = TRUE))%>% 
  ungroup()

part1_final_ww1$compound <- as.character(part1_final_ww1$compound)
part1_final_ww1$Group <- as.character(part1_final_ww1$Group)

unique(part1_final_ww1$Sample.type)
#rename
part1_final_ww1$Sample.type <- revalue(part1_final_ww1$Sample.type, c("Water. filtered" = "Water, filtered (ng/L)", "Water. particulate" = "Water, particulate (ng/g)", "Sludge" ="Sludge (ng/g)"))

#set order
part1_final_ww1$Sample.type <- factor(part1_final_ww1$Sample.type, levels=c("Water, filtered (ng/L)", "Water, particulate (ng/g)", "Sludge (ng/g)", "Marine sediment (ng/g)", "Blue mussel (ng/g)")) 
#Fjern alle < LOD
part1_final_ww2 <- subset(part1_final_ww1, meanx != "NaN") 


#remove the duplicated values, but value and plotting are different so must first be removed

part1_final_ww3 <- distinct(part1_final_ww2, meanx, Sample.type, compound, Treatmen, .keep_all= TRUE) 

#part1_final_ww3<- part1_final_ww2x[!duplicated(part1_final_ww2x), ]

tri3 <- subset(part1_final_ww1, Group=="Bisphenol")

ggplot(part1_final_ww3, aes(y=log(meanx), x=compound, fill=Treatmen))+
...
part1_filt_du1 <- part1_freq_du %>% 
  mutate(plotting = ifelse(det_frac == 0, 
                           "< LOD", 
                           value))

part1_filt_du <- cbind(part1_filt_du1[,c(1:23)], apply(part1_filt_du1[,24, drop=F],2, dLimity))
...
part1_final_du$compound <- as.character(part1_final_du$compound)
part1_final_du$Group <- as.character(part1_final_du$Group)
part1_final_du$Treatmen <- as.character(part1_final_du$Treatmen)

#set order (also for products used blow)
part1_final_du$Treatmen <- factor(part1_final_du$Treatmen, levels=c("Recycling facility", "Private home", "boat care products", "Detergents for textiles", "EE products", "Fabric clothes", "Fabric furniture", "Paint and varnish", "Toys" )) 

#Fjern alle < LOD
part1_final_du2 <- subset(part1_final_du, meanx != "NaN") 

#dust <- subset(part1_final_du, Sample.type == "")

dust <- ggplot(subset(part1_final_du2, Sample.type%in% c("Dust")), aes(y=log(meanx), x=compound, fill=Treatmen))+
...
unknown <- subset(part1_filt_du1, Category =="Unknown hotspot")
#gjennomsnitt på: Treatmen, compound, Sample.type
products <- ggplot(subset(part1_final_du2, Category%in% c("Unknown hotspot")), aes(y=log(meanx)+10, x=compound, fill=Treatmen))+
  #geom_col(position="dodge")+
  geom_col(position = position_dodge2(width = 0.9, preserve = "single"))+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
...
ggplot(topP3, aes(x=compound, y=(plotting), colour=Level))+
  geom_point(size=3)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
