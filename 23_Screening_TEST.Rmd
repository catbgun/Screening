...
#PLOTTING
WS_23 <- ggplot(part2_finalxy24x, aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
  geom_tile(aes(fill = det_frac), colour="black")+  # background colours are mapped according to the value column
...
UF2 <- ggplot(part2_UF_2x, aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
 ...
 scale_fill_gradient2(low = "#74C476", 
  ...
  theme(panel.grid.major.x=element_blank(), #no gridlines
        ...
        plot.margin = margin(1.3, 0.2, 0.15, 0.15, "cm"), #top, right, bottom, left
        legend.margin=margin(0,0,0,0),
        legend.box.margin=margin(10,10,120,10),
        strip.text.x = element_text(size = 12),
        panel.spacing.x = unit(0.2, "lines"),
        legend.title=element_text(size=13), 
        strip.text.y = element_text(angle = 0, size=10)) +
...
part1_filt_ww1 <- cbind(part1_filt_ww[,c(1:43)], apply(part1_filt_ww[,44, drop=F],2, dLimity))
...
#rename
unique(part1_final_ww2$Sample.type)
part1_final_ww2$Sample.type <- revalue(part1_final_ww2$Sample.type, c("Water, filtered" = "Water, filt. (ng/L)", "Water, particulate" = "Water, part. (ng/g, d.w.)", "Sludge" ="Sludge (ng/g, d.w.)"))

#set order
part1_final_ww2$Sample.type <- factor(part1_final_ww2$Sample.type, levels=c("Water, filt. (ng/L)", "Water, part. (ng/g, d.w.)", "Sludge (ng/g, d.w.)")) 
#Fjern alle < LOD
...
part1_final_ww4$Treatmen = factor(part1_final_ww4$Treatmen, levels = c("Low purification", "High purification") )

WWT_treatment <- ggplot(part1_final_ww4, aes(y=log(meanx)+5, x=compound, fill=Treatmen))+
...
dust3 <- cbind(dust2[,c(1:43)], apply(dust2[,44, drop=F],2, dLimity))
...
prod3 <- cbind(prod2[,c(1:43)], apply(prod2[,44, drop=F],2, dLimity))
...
products <- ggplot(prod5, aes(y=log(meanx), x=compound, fill=Treatmen))+
  #geom_col(position="dodge")+
...
```{r trophic level}
topP3 <- subset(part2_final_2, Sample.type =c("Blubber", "Liver"))
#rename
...
#remove nas
topP5 <- subset(topP4, plotting != "NaN") 

ggplot(topP5, aes(x=compound, y=log(plotting)+10, colour=Level))+
  geom_point(size=2.6)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
...
-whale species
```{r}
topP6 <- subset(topP4, Treatmen == "Whales" & Sample.type=="Blubber")

#remove nas
topP7 <- subset(topP6, plotting != "NaN") 

unique(topP6$Category)
topP7$Category <- factor(topP7$Category, levels=c("Killer whale", "Sperm whale", "Fin whale", "Humpback whale", "White beaked dolphin", "Harbor porpoise")) 

w_species <- ggplot(topP7, aes(x=compound, y=log(plotting)+10, colour=Category))+
  geom_point(size=2.6)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
...
```{r}
topP6x <- subset(topP4, Category=="Harbor porpoise"&Length==136)
#remove nas
topP7x <- subset(topP6x, plotting != "NaN") 


w_tissue <- ggplot(topP7x, aes(x=compound, y=log(plotting)+10, shape=Sample.type, colour= Sample.type))+
  geom_point(size=4)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
...
ggplot(topP4, aes(x=compound, y=log(plotting)+10, colour=Level))+
  geom_point(size=2.6)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
...
topP3xx <- subset(topP4, Level ==c("Whale top (blubber)"))

topP3xx$Category <- factor(topP3xx$Category, levels=c("Killer whale", "Sperm whale", "White beaked dolphin", "Harbor porpoise")) 

whale_top <- ggplot(topP3xx, aes(x=compound, y=log(plotting)+10, colour=Category))+
  geom_point(size=3)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
 ...
 ggsave("Whales TOP.png", whale_top, width = 10, height = 8) 
