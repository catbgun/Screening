...
df23 <- read.table("240815_240528_SCRE2023_Resultater.txt", skip=0, header=TRUE, sep="\t", na.string=c(""))
```
...
# claculate 0.5xLOD 
part1_filt_X <- cbind(part1_filt[,c(1:42)], apply(part1_filt[,43, drop=F],2, dLimity))
...
part2_filt_X <- cbind(part2_filt[,c(1:43)], apply(part2_filt[,44, drop=F],2, dLimity))
```
...
part1_finalxy$compound <- revalue(part1_finalxy$compound, c("BENAZOL.P"="BENAZOL P", "X23BPDT"="23BPDT", "X34DMPZ"="34DMPZ", "X2BBTMP"="2BBTMP", "X34DMPZP"="34DMPZP", "X35DMPZ"="35DMPZ", "X2BDOMP"="2BDOMP", "X4NAS"="4NAS", "B.2ETF"="B-2ETF", "X2BHMPTP"="2BHMPTP", "X2BMPP"="2BMPP", "X2BMTP"="2BMTP", "X22BPDT"="22BPDT", "X2TBCLBEP"="2TBCLBEP" ))
...
# claculate 0.5xLOD for those with detection frequency above 30%
part2_filt_X_2 <- cbind(part2_filt_2[,c(1:43)], apply(part2_filt_2[,44, drop=F],2, dLimity))
...
ggplot(KW, aes(x=compound, y=log(plotting), fill=newtable$Maturity))+
  geom_point(size=3)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
