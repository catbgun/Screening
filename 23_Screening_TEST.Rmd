...
gr23 <- read.table("Scre23_Groupings4.txt", skip=0, header=TRUE, sep="\t", na.string=c(""))
...
newtable$compound <- revalue(newtable$compound, c("AG"="Ag", "AS"="As", "CD"="Cd", "CR"="Cr", "CU"="Cu", "FE"="Fe", "HG"="Hg", "NI"="Ni", "PB"="Pb", "SB"="Sb", "SE"="Se", "SN"="Sn", "ZN"="Zn", "DMPSHEN"="OTNE"))
...
print(unique(newtable$Category))
...
# claculate 0.5xLOD 
part1_filt_X <- cbind(part1_filt[,c(1:48)], apply(part1_filt[,49, drop=F],2, dLimity))
...
part2_filt_X <- cbind(part2_filt[,c(1:48)], apply(part2_filt[,49, drop=F],2, dLimity))
```
...
ttt <- subset(part2_finalxy24xx, Sample.type=="Muscle (ng/g, w.w.)")
...
WS_23 <- ggplot(part2_finalxy24xx, aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
 ...
 scale_fill_gradient2(low = "#74C476", 
  ...
  theme(panel.grid.major.x=element_blank(), #no gridlines
        ...
        legend.title=element_text(size=12), 
        strip.text.y = element_text(angle = 0, size=12)) +
  ggtitle("Whales and sharks (emerging)") + 
  theme(legend.title=element_text(size=12)) + 
  scale_x_discrete(name="") +
  scale_y_discrete(name="") +
  labs(fill="Detection frequency (%)")+
...
part2_UF_3 <- subset(part2_extra1, Group %in%c("PFCA", "PFSA", "QAC", "UV") | compound=="Hg")

part2_UF_met <- subset(part2_extra1, compound %in%c("Cr", "Fe", "Ni", "Cu", "Zn", "As", "Se", "Ag", "Cd", "Sn", "Sb", "Pb"))

part2_UF_1x <- subset(part2_UF_1, det_frac != 0) 
part2_UF_2x <- subset(part2_UF_2, det_frac != 0) 
part2_UF_3x <- subset(part2_UF_3, det_frac != 0) 

part2_UF_metx <- subset(part2_UF_met, det_frac != 0) 
...
UF1 <- ggplot(part2_UF_1x, aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
 ...
 scale_fill_gradient2(low = "#74C476", 
  ...
  theme(panel.grid.major.x=element_blank(), #no gridlines
        ...
        legend.title=element_text(size=12), 
        strip.text.y = element_text(angle = 0, size=12)) +
  ggtitle("Whales and sharks (legacy 1/3)") + 
  theme(legend.title=element_text(size=12)) + 
  scale_x_discrete(name="") +
  scale_y_discrete(name="") +
  labs(fill="Detection frequency (%)")+
...
UF2 <- ggplot(part2_UF_2x, aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
 ...
 theme(panel.grid.major.x=element_blank(), #no gridlines
        ...
        legend.title=element_text(size=12), 
        strip.text.y = element_text(angle = 0, size=12)) +
  ggtitle("Whales and sharks (legacy 2/3)") + 
  theme(legend.title=element_text(size=12)) + 
  scale_x_discrete(name="") +
  scale_y_discrete(name="") +
  labs(fill="Detection frequency (%)")+
...
UF3 <- ggplot(part2_UF_3x, aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
 ...
 scale_fill_gradient2(low = "#74C476", 
   ...
   theme(panel.grid.major.x=element_blank(), #no gridlines
        ...
        legend.title=element_text(size=12), 
        strip.text.y = element_text(angle = 0, size=12)) +
  ggtitle("Whales and sharks (legacy 3/3)") + 
  theme(legend.title=element_text(size=12)) + 
  scale_x_discrete(name="") +
  scale_y_discrete(name="") +
  labs(fill="Detection frequency (%)")+
...
#metals except Hg - for appendix
#part2_UF_metx

metx <- ggplot(part2_UF_metx, aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
  geom_tile(aes(fill = det_frac), colour="black")+  # background colours are mapped according to the value column
  geom_text(aes(label= ifelse(meanxyz=="S", meanxyz, ifelse(is.na(meanx), " ", as.numeric(sigfig(meanx, 1))))), color = "white", size = 5)+
  #geom_text(aes(label=ifelse(is.na(meanx), "", sigfig(meanx, 1))), color = "white", size = 4)+
 scale_fill_gradient2(low = "#74C476", 
   ...
   theme(panel.grid.major.x=element_blank(), #no gridlines
        ...
        legend.title=element_text(size=12), 
        strip.text.y = element_text(angle = 0, size=12)) +
  ggtitle("Whales and sharks (metals, except Hg)") + 
  theme(legend.title=element_text(size=12)) + 
  scale_x_discrete(name="") +
  scale_y_discrete(name="") +
  labs(fill="Detection frequency (%)")+
...
ggsave("Part1_UF3_metals.png", metx, width = 8, height = 8)
...
part1_filt_ww1 <- cbind(part1_filt_ww[,c(1:48)], apply(part1_filt_ww[,49, drop=F],2, dLimity))
...
part1_final_ww5$compound <- as.character(part1_final_ww5$compound)

WWT_treatment <- ggplot(part1_final_ww5, aes(y=(meanx), x=(compound), fill=Treatmen))+
...
dust3 <- cbind(dust2[,c(1:48)], apply(dust2[,49, drop=F],2, dLimity))
...
dust <- ggplot(dust6, aes(y=meanx, x=compound, fill=Treatmen))+
  geom_col(position = position_dodge2(width = 0.9, preserve = "single"))+
...
prod3 <- cbind(prod2[,c(1:48)], apply(prod2[,49, drop=F],2, dLimity))
...
- figure for Hg in muscle 
```{r Hg figure}
Hg <- subset(part2_final_3, compound %in% c("Hg"))
class(Hg$plotting)
Hg$plotting <- as.character(Hg$plotting)
Hg1 <- subset(Hg, plotting != "NaN")
#Hg1$plotting1 <- Hg1$plotting:1000

Hg$Category <- revalue(Hg$Category, c("H???brann" = "Porbeagle shark", "Piggh???" = "Spiny dogfish", "sperm whale" = "Sperm whale", "H???kjerring"="Greenland shark"))

Hg$Category <- factor(Hg$Category, levels=c("Killer whale","Sperm whale", "Fin whale", "Humpback whale", "White beaked dolphin", "Harbor porpoise", "Greenland shark", "Porbeagle shark", "Spiny dogfish")) 


ggplot(Hg, aes(x=Category, y=as.numeric(value), shape=Sample.type, colour=Sample.type))+
  geom_point(size=3.5)+
  #geom_hline(yintercept=16000, colour="red", linewidth=1.5)+
  theme_bw()+
   theme(axis.text.x = element_text(angle = 45, size=11, colour="black", hjust=1),
        axis.text.y = element_text(size=12, colour="black"),
        axis.title.x= element_blank(),
        axis.title.y = element_text(size=12, margin = margin(t = 0, r = 20, b = 0, l = 0)),
        strip.text.x = element_text(angle = 90, hjust=0, size=15, colour="black"),
        ...
        #legend.position="top",
        legend.title=element_blank())+
  ylab("Concentration Hg (ng/g, w.w.)")+
...
ggsave("Hg figure.png", Hg_fig, width = 6, height = 5)


```

- whales (blubber) and sharks (liver) - trophic level - not used!
```{r trophic level}
topP3 <- subset(part2_final_3, Sample.type %in% c("Blubber", "Liver"))
#rename
...
- all whale and shark
```{r trophic level}
topP3 <- subset(part2_final_3, Sample.type %in% c("Blubber", "Liver", "Muscle"))
#rename
topP3$Level <- revalue(topP3$Level, c("Shark intermediate predator" = "Shark intermediate", "Shark top predator" = "Shark top", "Whale intermediate predator" = "Whale intermediate", "Whale top predator"= "Whale top"))

#set order (also for products used blow)
topP3$Level <- factor(topP3$Level, levels=c("Whale top","Shark top", "Whale intermediate", "Shark intermediate")) 
topP4 <- subset(topP3, !compound %in% c("Ag","As","Cd","Cr", "Cu","Fe","Ni","Pb","Sb","Se","Sn","Zn"))
...
topP6z$compound <- as.character(topP6z$compound)
whales_all <- ggplot(topP6z, aes(x=compound, y=plotting, colour=Level, shape=Sample.type))+
  geom_point(size=3.5)+
  theme_bw()+
  scale_y_continuous(trans='log10') +
  facet_grid(.~Group, scales="free", space='free_x')+
  ...
  ylab("Concentration (ng/g, w.w.)")+
  scale_colour_manual(name = "Tissue",
                      labels = c("Whale top","Shark top", "Whale intermediate", "Shark intermediate"),
                      values = c("dodgerblue3", "green3", "plum", "orange1"))+  
  scale_shape_manual(values = c(18, 17, 15))+ #17 er trekant
  guides(color=guide_legend("Sample.type"), shape = "none")
  guides(color = guide_legend(override.aes = list(size=4)))

  
ggsave("Whale groups_all.png", whales_all, width = 10, height = 8)
...
topP7xx <- subset(topP7x, compound != "Fett") 
options(scipen = 999)

 w_tissue <- ggplot(topP7xx, aes(x=compound, y=(plotting), shape=Sample.type, colour= Sample.type))+
  geom_point(size=4.5)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
...
- trophic level / fat (%) - all sample types!
```{r}
...
part30 = newtablex%>%
  mutate(Level = case_when(Category %in% c("Killer whale","Sperm whale", "Harbor porpoise", "White beaked dolphin") ~ "Whale top",
                           Category %in% c("Greenland shark", "Porbeagle shark") ~ "Shark top",
                                Category %in% c("Humpback whale", "Fin whale") ~ "Whale intermediate",
                           Category %in% c("Spiny dogfish") ~ "Shark intermediate"))

#set order (also for products used blow)
part30$Level <- factor(part30$Level, levels=c("Whale top","Shark top", "Whale intermediate", "Shark intermediate")) 
...
### PCB in killer whale
```{r}
KW <- subset(part33, Species =="Killer whale")
KW_pcb <- subset(KW, Group=="PCB")

KW_pcb1 = KW_pcb%>%
  mutate(ID = case_when(Length %in% c("390") ~ "OO11",
                           Length %in% c("392") ~ "005",
                                Length %in% c("740") ~ "OO14"))

KW_pcb <- ggplot(KW_pcb1, aes(x=ID, y=new, fill=compound))+
  geom_bar(position="stack", stat="identity")+
  geom_hline(yintercept=9000, colour="red", linewidth=1.5)+
  theme_bw()+
   theme(axis.text.x = element_text(angle = 90, size=12, colour="black"),
        axis.text.y = element_text(size=12, colour="black"),
        axis.title.x= element_blank(),
        axis.title.y = element_text(size=13, margin = margin(t = 0, r = 20, b = 0, l = 0)),
        strip.text.x = element_text(angle = 90, hjust=0, size=15, colour="black"),
        strip.text.y = element_text(size=12, colour="black"),
        panel.spacing.x = unit(0, "lines"),
        strip.background.x = element_blank(),
        ...
        #legend.position="top",
        legend.title=element_blank())+
  ylab("Lipid normalised concentration (ng/g, l.w.)")
  #scale_y_continuous(trans='log10') +
  #facet_grid(Sample.type~Group, scales="free", space='free_x')+
  
  #scale_fill_brewer(palette = "Set5")

ggsave("Killer whales_PCB.png", KW_pcb, width = 6, height = 4)
...
part32x <- subset(part32, Sample.type !="Blue mussel")

part32x$compound <- as.character(part32x$compound)
whales_all_FAT <- ggplot(part32x, aes(x=compound, y=new, colour=Level, shape=Sample.type))+
  geom_point(size=3.5)+
  theme_bw()+
  scale_y_continuous(trans='log10') +
  facet_grid(.~Group, scales="free", space='free_x')+
...
ggsave("Whale groups_all_FAT.png", whales_all_FAT, width = 10, height = 8)
```
...
```{r}
#the followin warning from NAs: Removed 32 rows containing missing value

ioh <- subset(part34x, Group!="Antioxidant") #beacuse only NA on age

as_FAT <- ggplot(ioh, aes(x=compound, y=new, colour=Maturity, shape=Maturity))+
  geom_point(size=4)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=10, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
...
Se på stabil isotop
```{r}
iso <- subset(part2_final_3, !compound %in% c("Ag","As","Cd","Cr", "Cu","Fe","Ni","Pb","Sb","Se","Sn","Zn"))

#set order (also for products used blow)
iso$Category<- factor(iso$Category, levels=c("White beaked dolphin", "Harbor porpoise", "Greenland shark","Porbeagle shark", "Spiny dogfish")) 

#topP4$X..C <- as.numeric(topP4$X..C)
#topP4$X..N <- as.numeric(topP4$X..N)
#topP4$W..C.N <- as.numeric(topP4$W..C.N)

topP4_x <- distinct(iso, Sample.type, W..C.N, Category, .keep_all= TRUE) 
topP4_y <- distinct(iso, Sample.type, X..C, Category, .keep_all= TRUE) 
topP4_z <- distinct(iso, Sample.type, X..N, Category, .keep_all= TRUE) 

CN <- ggplot(subset(topP4_x, !is.na(W..C.N)), aes(x=Category, y=W..C.N, fill=Treatmen))+
  geom_col(position = position_dodge2(width = 0.9, preserve = "single"))+
  #geom_point(size=5)+
  theme_bw()+
  theme(axis.text.x = element_text(size=13, colour="black", angle = 45, vjust = 1, hjust=1),
        axis.text.y = element_text(size=11, colour="black"), #axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
        axis.title.x= element_blank(),
  ...
  scale_fill_manual(values = c("darkolivegreen4", "bisque3"))

ggsave("CN.png", CN, width = 9, height = 5.5)
  
dC <- ggplot(subset(topP4_y, !is.na(X..C)), aes(x=Category, y=X..C, fill=Treatmen))+
   geom_col(position = position_dodge2(width = 0.9, preserve = "single"))+
  #geom_point(size=5)+
  theme_bw()+
  theme(axis.text.x = element_text(size=13, colour="black", angle = 45, vjust = 1, hjust=1),
...
ggsave("dC.png", dC, width = 9, height = 5.5)
  
dN <- ggplot(subset(topP4_z, !is.na(X..N)), aes(x=Category, y=X..N, fill=Treatmen))+
     geom_col(position = position_dodge2(width = 0.9, preserve = "single"))+
  #geom_point(size=5)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90, size=13, colour="black"),
        axis.text.y = element_text(size=11, colour="black"),
        axis.title.x= element_blank(),
        axis.title.y = element_text(size=14, margin = margin(t = 0, r = 20, b = 0, l = 0)),
        strip.text.x = element_text(angle = 90, hjust=0, size=11, colour="black"),
...
ggsave("dN.png", dN, width = 9, height = 5.5)
