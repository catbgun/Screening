...
#set the order
hotspot1 <- hotspot %>% 
  mutate(Sample.type = factor(Sample.type, levels = c("Private home, dust (ng/g)","Waste recycling, dust (ng/g)", "Commercial products (ng/g)", "Agricultural soil (ng/g)")))
...
#REVALUE
part2_finalxyz$Sample.type <- revalue(part2_finalxyz$Sample.type, c("Blubber" ="Blubber (ng/g)", "Liver" = "Liver (ng/g)", "Muscle"="Muscle (ng/g)"))
#unique(part2_finalxy23x$Sample.type)
#unique(part2_finalxyz$Category) 

part2_finalxyz$Category <- revalue(part2_finalxyz$Category, c("H?brann" = "Porbeagle shark", "Piggh?" = "Spiny dogfish", "sperm whale" = "Sperm whale", "H?kjerring"="Greenland shark"))
...
unique(part2_finalxy23x$Category) 

#PLOTTING
WS_23 <- ggplot(subset(part2_finalxy23x, Treatmen%in% c("Whales", "Sharks")), aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
  geom_tile(aes(fill = det_frac), colour="black")+  # background colours are mapped according to the value column
  geom_text(aes(label=ifelse(is.na(meanx), "", sigfig(meanx, 1))), color = "white", size = 5)+
 scale_fill_gradient2(low = "green", 
                       mid = "orange", 
...
ggsave("WS_23.png", WS_23, width = 8, height = 6) 
```
...
part2_UF_3 <- subset(part2_finalxy23x, Group == c("PFCA", "PFSA", "QNH4", "UV"))
part2_UF_met <- subset(part2_finalxy23x, Group == c("Metal"))

part2_UF_met$compound <- revalue(part2_UF_met$compound, c("AG"="Ag", "AS"="As", "CD"="Cd", "CR"="Cr", "CU"="Cu", "FE"="Fe", "HG"="Hg", "NI"="Ni", "PB"="Pb", "SB"="Sb", "SE"="Se", "SN"="Sn", "ZN"="Zn"))

unique(part2_finalxy23$Group)

UFmet <-  ggplot(part2_UF_met, aes(x=Sample.type, y=fct_rev(compound), fill=det_frac)) +
  geom_tile(aes(fill = det_frac), colour="black")+  # background colours are mapped according to the value column
  geom_text(aes(label=ifelse(is.na(meanx), "", sigfig(meanx, 1))), color = "white", size = 5)+
 scale_fill_gradient2(low = "green", 
                       mid = "orange", 
...
#save
ggsave("Part1_UF1.png", UF1, width = 10, height = 10)
ggsave("Part1_UF2.png", UF2, width = 10, height = 12)
ggsave("Part1_UF3.png", UF3, width = 10, height = 10)
ggsave("Part1_met.png", UFmet, width = 10, height = 15)
